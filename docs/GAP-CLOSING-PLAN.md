...
- [ ] 网络隔离
- [ ] 可配置的允许/禁止路径和域名
- [ ] `autoAllowBashIfSandboxed` 沙箱内自动允许 Bash

### 9.3 上下文可视化
- [ ] `/context` 彩色网格显示 token 分布
- [ ] 各消息占用比例柱状图
- [ ] 技能/规则内容存活压缩

### 9.4 CLI 增强
- [ ] `--bare` 最小模式（跳过自动发现钩子/技能/插件/MCP）
- [ ] `--fallback-model` 自动回退模型
- [ ] `--tools <tools>` 限制可用内置工具
- [ ] `--output-format text|json|stream-json`
- [ ] `--add-dir` 添加额外工作目录
- [ ] `project purge` 删除项目所有 SparkCLI 状态
