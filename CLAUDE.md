...
Running `spark-cli` with no subcommand falls through to `runShell` (Claude-Code-style REPL with `/diff`, `/apply`, `/model …` slash commands). `spark-cli chat` with no prompt does the same.
...
- `src/mcp/` — MCP server exposing read/write tools to Cursor / Claude Desktop (`spark-cli mcp serve`), with `write-guard.ts` enforcing the staging invariant for tool-driven writes.
