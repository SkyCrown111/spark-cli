...
export async function runProfileCapture(
  ...
  for (const n of plan.notes) console.log(chalk.dim(`  · ${n}`));
  if (plan.command) console.log(chalk.dim(`  cmd: ${plan.command}`));
  return 0;
}

export async function runProfileAnalyze(
  opts: GlobalOptions,
  file: string,
): Promise<number> {
  const root = resolveProjectRoot(opts);
  ...
  console.log(`  avg: ${analysis.summary.avgFrameMs.toFixed(2)}ms`);
  console.log(`  p95: ${analysis.summary.p95FrameMs.toFixed(2)}ms`);
  for (const s of analysis.systems.slice(0, 8)) {
    console.log(`  · ${s.name}: avg ${s.avgMs.toFixed(2)}ms`);
  }
  return 0;
}
